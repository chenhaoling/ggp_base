...
public class MCTSandH extends SampleGamer {
    ...
    public void stateMachineMetaGame(long timeout) throws TransitionDefinitionException, MoveDefinitionException, GoalDefinitionException {
        ...
        List<Move> moves;
        if(theMachine.getRoleIndices().get(me) == 0) {
            //获取中心点位置
            moves = theMachine.getLegalMoves(getCurrentState(), me);
        }
        else {
            moves = theMachine.getLegalMoves(getCurrentState(), opponent);
        }
        int max = 0;
        for (Move i :
                moves) {
        ...
        if (moves.size() > 1) {
            ...
            if (moves.size() > 1) {
                try {
                    ...
                    System.out.println("MCTSandH num of test:" + loop_i);
